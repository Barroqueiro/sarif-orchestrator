...
def set_initial_state():
    """
    Set initial state of all tools as WAITTING
    """
    for items in globals()["runs"].values():
        items["state"] = states.WAITTING
...
def wait_for_threads_completion():
    """
...
def main():
    ...
    set_initial_state()       # Set all tool's states to WAITTING
    create_tasks()       # Create all threads (One for each tool)
    start_tasks()        # Start all threads
    wait_for_threads_completion()         # Wait for all threads to complete
